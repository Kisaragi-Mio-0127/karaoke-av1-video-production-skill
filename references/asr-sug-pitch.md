...
## SUG compatibility gate

- Read the application version from `src/strange_uta_game/__version__.py` and the storage format from `SugMigrator.CURRENT_VERSION`. Use those runtime values as the parser contract.
...
- Installation additionally requires the target application version and SUG format to match the installer's bundled compatibility constants. A successful parser load does not bypass that exact check.
...
- The shifted output inherits only safe descriptive tags. FLAC retains attached cover art; WAV receives equivalent descriptive labels without copying technical metadata.
