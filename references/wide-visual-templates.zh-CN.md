...
- `spectrum-line`固定包含40个频谱点，并在左右两端各增加一个固定`Y=0`的边界锚点，折线总计42个顶点。相邻点使用直线段连接；每个可见频谱点在相同`x`位置使用2像素宽、55%不透明度的竖线连接到归一化幅值`Y=0`。零值频谱点保持隐藏，避免形成底部横线。高度通道在插值期间保持16位；折线和竖线以`4160x880`绘制（4倍SSAA），再用Lanczos缩小到`1040x220`，得到半径1.25像素的抗锯齿线条。竖线颜色和辉光均延伸到`Y=0`，辉光在该坐标下方裁掉，避免进入进度条区域。
