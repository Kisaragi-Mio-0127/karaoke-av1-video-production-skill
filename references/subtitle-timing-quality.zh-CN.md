...
对于日文宽屏布局，将同一条带时间源行内部的空格视为显式语义或换气边界，而不是源歌词换行。当可见文本超过常规显示长度目标时，应在完整行宽度快速通过之前，先沿这些空格将相邻片段组合成长度均衡的显示短句；合并可避免的过短片段。较短的整行可以将每个源空格边界渲染为细微语义间距，但空格字符本身绝不能成为带时间或可高亮的字形事件。对于没有可用空格的源行，先测量完整行，再使用按字符数回退分割。绝不要在连续片假名串内部或规范`linked_to_next`注音跨度中切分，短片段再平衡也必须保留同一注音跨度边界。这些是语言规则，不是歌曲专属的显示覆盖。
