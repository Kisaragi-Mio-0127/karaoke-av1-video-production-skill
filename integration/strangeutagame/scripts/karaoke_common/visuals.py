    ...
    *,
    ...
    if visual_style == "spectrum-line":
        spectrum_scene = (
            ...
            "tmix=frames=5:weights='0.34 0.24 0.18 0.14 0.10',"
            "lagfun=decay=0.94,"
            "lut=y='if(lt(val\\,1024)\\,0\\,val)',"
    ...
    elif visual_style == "spectrum-ribbon":
        spectrum_scene = (
            f"[0:v]format=rgba,trim=start={start:.3f}:end={end:.3f},"
            "setpts=PTS-STARTPTS[bgclip];"
            f"[1:a]atrim=start={start:.3f}:end={end:.3f},"
            ...
            "st(2\\,min(876\\,940-p(4052\\,Y)/64.25));"
            "if(lt(X\\,107)\\,"
            "65535*clip(5-abs((ld(1)-876)*X-107*(Y-876))/"
            "sqrt((ld(1)-876)*(ld(1)-876)+11449)\\,0\\,1)\\,"
            "if(gt(X\\,4052)\\,"
            "65535*clip(5-abs((876-ld(2))*(X-4052)-107*(Y-ld(2)))/"
            "sqrt((876-ld(2))*(876-ld(2))+11449)\\,0\\,1)\\,"
            "65535*clip(5-abs(Y-ld(0))\\,0\\,1)))',"
            "scale=1040:220:flags=lanczos,format=gray,"
            "lut=y='if(lte(val\\,16)\\,0\\,val)',"
            "split=5[ribboncoremask][ribboninnermask][ribbonoutermask]"
            "[ribbontrailcoresrc][ribbontrailglowsrc];"
            "[ribboninnermask]pad=1168:348:64:64:color=black,"
            "gblur=sigma=3:steps=2,"
            "drawbox=x=0:y=284:w=1168:h=64:color=black:t=fill,"
            "lut=y='val*1.7'[ribboninner];"
            "[ribbonoutermask]pad=1168:348:64:64:color=black,"
            "gblur=sigma=10:steps=2,"
            "drawbox=x=0:y=284:w=1168:h=64:color=black:t=fill,"
            "lut=y='val*1.25'[ribbonouter];"
            "[ribbontrailcoresrc]tmix=frames=7:"
            "weights='1 0.80 0.62 0.45 0.30 0.18 0.10',"
            "lut=y='val*1.35'[ribbontrailcoremask];"
            "[ribbontrailglowsrc]tmix=frames=7:"
            "weights='1 0.80 0.62 0.45 0.30 0.18 0.10',"
            "pad=1168:348:64:64:color=black,gblur=sigma=5:steps=2,"
            "drawbox=x=0:y=284:w=1168:h=64:color=black:t=fill,"
            "lut=y='val*0.9'[ribbontrailglowmask];"
            f"color=c=0x{color}:s=1040x220:r=30:d={duration:.3f},"
            "format=rgba[ribboncorecolor];"
            f"color=c=0x{color}:s=1168x348:r=30:d={duration:.3f},"
            "format=rgba[ribboninnercolor];"
            f"color=c=0x{color}:s=1168x348:r=30:d={duration:.3f},"
            "format=rgba[ribbonoutercolor];"
            f"color=c=0x{progress}:s=1040x220:r=30:d={duration:.3f},"
            "format=rgba[ribbontrailcorecolor];"
            f"color=c=0x{progress}:s=1168x348:r=30:d={duration:.3f},"
            "format=rgba[ribbontrailglowcolor];"
            "[ribboncorecolor][ribboncoremask]alphamerge[ribboncore];"
            "[ribboninnercolor][ribboninner]alphamerge[ribboninnerglow];"
            "[ribbonoutercolor][ribbonouter]alphamerge[ribbonouterglow];"
            "[ribbontrailcorecolor][ribbontrailcoremask]"
            "alphamerge[ribbontrailcore];"
            "[ribbontrailglowcolor][ribbontrailglowmask]"
            "alphamerge[ribbontrailglow];"
            "[bgclip][ribbontrailglow]overlay=736:226:format=auto[ribbonbase];"
            "[ribbonbase][ribbontrailcore]overlay=800:290:format=auto[ribbontrail];"
            "[ribbontrail][ribbonouterglow]overlay=736:226:format=auto[ribbonhalo];"
            "[ribbonhalo][ribboninnerglow]overlay=736:226:format=auto[ribbonglow];"
            "[ribbonglow][ribboncore]overlay=800:290:format=auto[spectrumscene];"
        )
    else:
        spectrum_scene = (
        f"[0:v]format=rgba,trim=start={start:.3f}:end={end:.3f},"
