...
For `vinyl`, replace `<vinyl-only-arguments>` with `--vinyl <vinyl-png>`. For `spectrum`, omit it and optionally add `--spectrum-color RRGGBB --progress-color RRGGBB`. If timing overrides are used, pass `--timing-overrides <json>` and `--song-id <id>` together. Omit `--lossless-output` for disposable short previews when the companion is unnecessary; require it for formal full renders. The default full-program AV1 4:2:0 direct-render profile is 1920x1080 at 30 fps, `yuv420p`, BT.709, AV1 NVENC CQ44, `preset p7`, `tune hq`, VBR, full-resolution multipass, lookahead 32, spatial and temporal AQ, AQ strength 8, and GOP 240 after a successful hardware probe. The default compatibility MP4 uses AAC-LC 320k; optionally pair it with MKV lossless audio from a genuinely lossless source. Do not show elapsed time or playback-control buttons.
