...
用`ffprobe`验证CQ、multipass、lookahead、AQ、GOP、尺寸、帧率、像素格式和BT.709元数据。不确定目标设备是否支持AV1时，另产H.264兼容版本。默认兼容输出只有AAC-LC 320k的MP4，不生成MKV，也不执行完整解码。MKV必须通过`--lossless-companion`或底层显式`--lossless-output`主动选择，并且只接受探测确认为FLAC或PCM WAV的源；MP3/AAC必须拒绝，不得把MP4 AAC转成无损音轨。
...
- 完整空解码默认关闭，是可选诊断而不是发布强制门禁，只在用户要求或探测、封装、传输、损坏证据需要时执行。仅缺少完整解码证据不得阻止发布、降低验证状态，也不得要求每个成品都具备解码退出码。执行时记录每个真实退出码；未执行既不是成功也不是失败。
