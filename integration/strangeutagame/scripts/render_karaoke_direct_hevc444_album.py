...
try:
    from . import karaoke_direct_album_planning as album_planning
    ...
    import karaoke_direct_album_planning as album_planning  # type: ignore[no-redef]
...
# Compatibility for callers that imported the implementation before its rename.
...
def plan_tasks(
    ...
    *,
    ...
    """Build and validate the manifest-selected direct-render task inputs."""
...
# The neutral module is authoritative for manifest selection and task planning.
# These aliases retain the implementation module's historical public API.
RenderTask = album_planning.RenderTask  # noqa: F811
select_tracks = album_planning.select_tracks  # noqa: F811
select_profiles = album_planning.select_profiles  # noqa: F811
find_latest_sug = album_planning.find_latest_sug  # noqa: F811
find_latest_ass = album_planning.find_latest_ass  # noqa: F811
resolve_font_paths = album_planning.resolve_font_paths  # noqa: F811
plan_tasks = album_planning.plan_tasks  # noqa: F811
...
def build_hevc444_report(
    ...
    *,
    ...
    """Build the aggregate HEVC 4:4:4 report for a complete manifest batch."""

    if not results:
        raise DirectAV1RenderError(
            "aggregate HEVC 4:4:4 report requires at least one song"
        )
    output_keys = {
    ...
    expected_result_count = len(PROFILES) * len(song_ids)
    if len(results) != expected_result_count:
        raise DirectAV1RenderError(
            "aggregate HEVC 4:4:4 report requires one result per song/profile pair"
        )
    profile_song_ids = {
        profile: {
    ...
    if any(len(ids) != len(song_ids) for ids in profile_song_ids.values()):
        raise DirectAV1RenderError(
            "aggregate HEVC 4:4:4 report must contain every song in both profiles"
        )
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="album.json manifest that defines the complete album batch",
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="select a song; repeat or comma-separate selectors (default: all)",
    )
    parser.add_argument(
        "--profile",
        "--profiles",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "revalidate every published direct HEVC 4:4:4 render and rebuild "
            "hevc444_report.json"
        ),
    )
    ...
    try:
        ...
        if args.report_only:
            if len(tracks) != len(album.tracks) or set(profiles) != set(PROFILES):
                raise ValueError(
                    "--report-only requires every manifest song and both profiles"
                )
