...
From the StrangeUtaGame repository root, build the static composition with `scripts/build_karaoke_wide_artwork.py`:
...
For `vinyl`, replace `<vinyl-only-arguments>` with `--vinyl <vinyl-png>`. For `spectrum`, omit it and optionally add `--spectrum-color RRGGBB --progress-color RRGGBB`. If timing overrides are used, pass `--timing-overrides <json>` and `--song-id <id>` together. Omit `--lossless-output` for disposable short previews when the companion is unnecessary; require it for formal full renders. A full-program AV1 4:2:0 direct render must explicitly use `--start 0`, the verified program duration, and `--video-encoder av1_nvenc --av1-cq <0..63>` after a successful hardware probe; the script's default is H.264 and its current `libaom-av1` lane emits `gbrp`, not the 4:2:0 release profile. The MP4 remains the default output and uses AAC-LC 320k; the MKV copies that video stream and takes FLAC directly from a probed lossless source. Do not show elapsed time or playback-control buttons.
...
- Keep the common lower subtitle backdrop at corner coordinates `(x1,y1,x2,y2)=(20,576,1900,1050)` unless a newly reviewed layout intentionally replaces it. Subtitle anchors come from `--layout`, not from this rectangle.
- For `vinyl`, use album-card geometry `(x,y,width,height)=(40,30,340,402)` and title block visual left edge `430`.
- For `spectrum`, use album-card geometry `(40,30,460,522)`, title/spectrum/progress visual left edge `800`, spectrum geometry `(x,y,width,height)=(800,290,1040,220)`, baseline `y=516`, and progress geometry `(800,548,1040,6)` with a 20 px circular indicator.
