...
在每一层记录生成ID或等价证据。对于专用日文MMS，任何`*_sha256`值仅用于报告，绝不能作为检查、门禁、例外或退出输入。规范SUG是可编辑的事实来源：候选生成填充缺失注音，可选审核可以写入已接受的修正，渲染读取选定项目。显示短句必须逐字准确地重新组成每个归一化源行。使用完整源行以及稳定的segment或occurrence身份标识覆盖规则，断言命中次数，并拒绝不可达、重复或过宽的规则。

将较长但语义完整的短语记录为已审核例外，并在目标字号和尺寸下验证视觉适配。只在获批准的换气点或语义边界添加语义间距，同时记录字符索引以及一个像素或em增量。

对于日文宽屏布局，将同一条带时间源行内部的空格视为显式语义或换气边界，而不是源歌词换行。当可见文本超过常规显示长度目标时，应在完整行宽度快速通过之前，先沿这些空格将相邻片段组合成长度均衡的显示短句；合并可避免的过短片段。较短的整行可以将每个源空格边界渲染为细微语义间距，但空格字符本身绝不能成为带时间或可高亮的字形事件。对于没有可用空格的源行，先测量完整行，再使用按字符数回退分割。绝不要在连续片假名串内部或规范`linked_to_next`词组中切分，短片段再平衡和显式显示覆盖也必须保留同一词组边界。这些是语言规则，不是歌曲专属的显示覆盖。
