...
歌曲专用的显示、注音分组和时间读音决定可分别通过`KARAOKE_DISPLAY_OVERRIDES`、`KARAOKE_RUBY_GROUP_OVERRIDES`和`KARAOKE_TIMING_READING_OVERRIDES`提供。日语工作流提供`optional`、`required`和`off`三种注音模式，默认为`optional`。
