    ...
    *,
    ...
    if visual_style == "spectrum-line":
        spectrum_scene = (
            ...
            "format=gray16le,scale=40:1:flags=area,"
            "scale=4160:880:flags=bilinear,format=gray16le,"
            "geq=lum='st(0\\,940-p(X\\,Y)/64.25);"
            "st(1\\,if(lt(X\\,52)\\,"
            "clip(5-abs((ld(0)-879)*X-52*(Y-879))/"
            "sqrt((ld(0)-879)*(ld(0)-879)+2704)\\,0\\,1)\\,"
            "if(gt(X\\,4108)\\,"
            "clip(5-abs((879-ld(0))*(X-4108)-51*(Y-ld(0)))/"
            "sqrt((879-ld(0))*(879-ld(0))+2601)\\,0\\,1)\\,"
            "clip(5-abs(Y-ld(0))\\,0\\,1))));"
            "65535*max("
            "ld(1)*max(gt(p(X\\,Y)\\,5140)\\,"
            "max(lt(X\\,52)\\,gt(X\\,4108)))\\,"
            "0.55*lte(mod(X+52\\,104)\\,7)*"
