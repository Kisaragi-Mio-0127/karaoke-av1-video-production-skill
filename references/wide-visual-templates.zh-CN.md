...
- `spectrum-line`固定包含40个频谱点。相邻点使用直线段连接；每个可见点在相同`x`位置使用2像素宽、55%不透明度的竖线连接到归一化幅值`Y=0`。零值点保持隐藏，避免形成底部横线；折线核心使用半径1.25像素的解析像素覆盖率抗锯齿。竖线颜色和辉光均延伸到`Y=0`，辉光在该坐标下方裁掉，避免进入进度条区域。
