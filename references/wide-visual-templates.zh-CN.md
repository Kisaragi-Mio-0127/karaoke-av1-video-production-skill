...
- 频谱零幅值基线：`y=516`。`spectrum-line`仅将其用作填充边界，不绘制可见的底部水平线；底部4像素范围内的轮廓采样点也会隐藏，避免零值数据形成横线。
- `spectrum-line`显示连续高亮轮廓，并用半透明区域向基线闭合。该基线表示归一化幅值`Y=0`，轮廓下方的填充区域不包含柱间空隙。相邻的全宽频谱采样点使用直线段连接；核心折线禁止缩放、平滑和膨胀处理，模糊仅用于独立的外围辉光层。
